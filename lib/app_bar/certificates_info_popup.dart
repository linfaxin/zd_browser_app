...
class _CertificateInfoPopupState extends State<CertificateInfoPopup> {
                    ...
                    children: <Widget>[
                      ...
                      Row(
                        ...
                        children: <Widget>[
                          Expanded(
                            child: Text(
                              "Flutter 浏览器已验证网站证书由 ${_topMainCertificate?.issuer(dn: ASN1DistinguishedNames.COMMON_NAME)} 签发。",
                              softWrap: true,
  ...
  List<Widget> _buildIssuedBySection(X509Certificate x509certificate) {
    ...
    var serialNumber = x509certificate.serialNumber
        ...
        "（不在证书中）";
    var version =
        x509certificate.version?.toString() ?? "（不在证书中）";
    var sigAlgName = x509certificate.sigAlgName ?? "（不在证书中）";
  ...
  List<Widget> _buildPublicKeySection(X509Certificate x509certificate) {
    ...
    var publicKeyAlg = "（不在证书中）";
    var publicKeyAlgParams = "（不在证书中）";
    if (publicKey != null) {
